...
class CropWindow(ctk.CTkToplevel):
    def __init__(self, parent, pil_image):
        ...
        self.grab_set()
        self.canvas = Canvas(self, cursor="cross", highlightthickness=0)
        self.canvas.pack(fill="both", expand=True)
        self.button_frame = ctk.CTkFrame(self)
        self.button_frame.pack(fill="x", padx=10, pady=10)
        self.apply_button = ctk.CTkButton(self.button_frame, text=parent._("apply_button"), command=self.apply_crop)
        self.apply_button.pack(side="right", padx=5)
        self.cancel_button = ctk.CTkButton(self.button_frame, text=parent._("cancel_button"), fg_color="gray", command=self.destroy)
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.top_frame.grid(row=0, column=0, columnspan=2, padx=20, pady=(20, 10), sticky="ew")
        
        self.lang_options = {"Türkçe": "tr", "English": "en", "Italiano": "it", "日本語": "jp", "Français": "fr", "Русский": "ru", "中文": "zh"}
        self.language_menu = ctk.CTkOptionMenu(self.top_frame, values=list(self.lang_options.keys()), command=self.change_language)
        self.language_menu.pack(side="left", padx=10, pady=10)

        self.donate_button = ctk.CTkButton(self.top_frame, text="", fg_color=DONATE_RED, command=self.open_donate_window)
        self.donate_button.pack(side="right", padx=10, pady=10)
        ...
        self.preview_label.place(in_=self.preview_frame, relx=0.5, rely=0.5, anchor="center")

        self.edit_frame = ctk.CTkFrame(self.left_frame)
        self.edit_frame.grid(row=1, column=0, padx=10, pady=10, sticky="ew")
        self.edit_frame.grid_columnconfigure((0, 1, 2, 3), weight=1)

        self.edit_label = ctk.CTkLabel(self.edit_frame, text="", font=ctk.CTkFont(weight="bold"))
        self.edit_label.grid(row=0, column=0, columnspan=4, pady=(5, 10))
        ...
        self.controls_frame.grid(row=1, column=1, padx=(10, 20), pady=10, sticky="nsew")
        
        self.image_button = ctk.CTkButton(self.controls_frame, command=self.select_image, fg_color=STEAM_BLUE)
        self.image_button.pack(pady=(20, 5), padx=20, fill="x")
        self.image_path_var = ctk.StringVar()
        self.image_label = ctk.CTkLabel(self.controls_frame, textvariable=self.image_path_var, wraplength=320, font=ctk.CTkFont(size=11))
        self.image_label.pack(pady=(0, 10), padx=20)

        self.profile_label = ctk.CTkLabel(self.controls_frame, font=ctk.CTkFont(size=12, weight="bold"))
        self.profile_label.pack(pady=(10, 0), padx=20, fill="x")
        self.profile_combobox = ctk.CTkComboBox(self.controls_frame, state="disabled", command=self.on_profile_select)
        self.profile_combobox.pack(pady=5, padx=20, fill="x")
        
        self.scan_button = ctk.CTkButton(self.controls_frame, command=self.find_and_list_games)
        self.scan_button.pack(pady=10, padx=20, fill="x")
        self.game_combobox = ctk.CTkComboBox(self.controls_frame, state="disabled", command=self.on_game_select)
        self.game_combobox.pack(pady=5, padx=20, fill="x")
        
        self.manual_folder_button = ctk.CTkButton(self.controls_frame, command=self.select_folder, fg_color="gray")
        self.manual_folder_button.pack(pady=5, padx=20, fill="x")
        self.folder_path_var = ctk.StringVar()
        self.folder_label = ctk.CTkLabel(self.controls_frame, textvariable=self.folder_path_var, wraplength=320, font=ctk.CTkFont(size=11))
        self.folder_label.pack(pady=(0, 10), padx=20)
        
        self.progress_frame = ctk.CTkFrame(self.controls_frame, fg_color="transparent")
        self.progress_frame.pack(pady=5, padx=20, fill="x", expand=True)
        self.progress_label = ctk.CTkLabel(self.progress_frame, text="")
        self.progress_bar = ctk.CTkProgressBar(self.progress_frame)
        self.progress_bar.set(0)

        self.process_button = ctk.CTkButton(self.controls_frame, font=ctk.CTkFont(size=18, weight="bold"), height=50, command=self.run_process)
    ...
    def update_ui_language(self, lang_code):
        ...
        if not self.current_pil_image and not self.image_paths_list: self.preview_label.configure(text=self._("preview_placeholder"))
    ...
    def open_donate_window(self):
        donate_win = ctk.CTkToplevel(self)
        donate_win.title(self._("donate_window_title"))
        donate_win.geometry("450x200")
        donate_win.resizable(False, False)
        donate_win.grab_set()

        main_text = ctk.CTkLabel(donate_win, text=self._("donate_main_text"), font=ctk.CTkFont(size=14))
        main_text.pack(pady=15, padx=20)
        ...
        iban_entry = ctk.CTkEntry(iban_frame, width=300)
        iban_entry.insert(0, self.iban_string)
        iban_entry.configure(state="readonly")
        iban_entry.pack(side="left", padx=10, fill="x")
        
        copy_button = ctk.CTkButton(donate_win, text=self._("donate_copy_button"), command=self.copy_iban)
        copy_button.pack(pady=15)

    def copy_iban(self):
        self.clipboard_clear()
        self.clipboard_append(self.iban_string)
    ...
    def load_profiles(self):
        ...
        if profiles:
            ...
            if last_profile and last_profile in profile_names:
                self.profile_combobox.set(last_profile)
            else:
                initial_profile = profile_names[0]
                self.profile_combobox.set(initial_profile)
                self.on_profile_select(initial_profile)
        else:
            self.profile_combobox.set(self._("no_profiles_found"))
    ...
    def on_profile_select(self, selected_profile):
        ...
        self.game_combobox.configure(values=[self._("game_combobox_default")], state="disabled")
        self.games_data = {}
    ...
    def find_and_list_games(self):
        ...
        if not selected_user_id:
            self.show_message({"success": False, "message_key": "no_profiles_found"})
            return
    ...
    def select_image(self):
        ...
        if len(self.image_paths_list) == 1:
            self.full_image_path = self.image_paths_list[0]
            self.image_path_var.set(os.path.basename(self.full_image_path))
            try:
                self.original_pil_image = Image.open(self.full_image_path)
            ...
            self.preview_label.configure(image=None, text=self._("multiple_images_selected", len(self.image_paths_list)))
    ...
    def update_preview(self):
        ...
        try:
            ...
            img_copy.thumbnail((self.preview_frame.winfo_width() - 20, self.preview_frame.winfo_height() - 20))
            ctk_img = ctk.CTkImage(light_image=img_copy, dark_image=img_copy, size=img_copy.size)
            self.preview_label.configure(image=ctk_img, text="")
        except Exception: self.preview_label.configure(image=None, text="Preview Error")
    ...
    def run_process(self):
        if not self.image_paths_list:
            self.show_message({"success": False, "message_key": "select_image_warning"})
            return
        
        folder_path = self.folder_path_var.get()
        if self._("no_folder_selected") in folder_path or not folder_path or not os.path.exists(folder_path):
            self.show_message({"success": False, "message_key": "select_folder_warning"})
            return
        ...
        for index, path in enumerate(self.image_paths_list):
            ...
            source = self.current_pil_image if total_count == 1 and self.current_pil_image else path
            process_image(source, folder_path)
    ...
    def toggle_ui_elements(self, enabled=True):
        ...
        widgets = [self.image_button, self.profile_combobox, self.scan_button, 
                   self.game_combobox, self.manual_folder_button, self.process_button,
